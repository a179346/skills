...
├── SKILL.md
└── references/
    └── writing-guide.md    — skill-writing 的完整指引（單一來源）
```
...
## 階段 1: 可行性檢查（Sanity Check）

在開始之前，做一個快速的可複用性檢查。一個好的 skill 候選流程應該：

- **會被重複使用** — 跨專案、跨 repo、或隨時間重複。一次性任務（修一個特定 bug、一次性資料遷移）不需要變成 skill。
- **步驟可泛化** — 核心流程在這次特定情境之外也能運作。如果每個步驟都依賴這個 codebase/dataset/context，那就不可複用。
- **一致性有價值** — 每次照同樣步驟做很重要（例如 deploy checklist、review 流程）。

如果流程看起來是一次性的，簡短提醒：「這看起來比較像一次性的操作 — 你確定要做成可複用的 skill 嗎？」如果使用者仍然想要，繼續執行 — 他們可能看到你沒看到的複用潛力。
...
## 階段 3: 載入 skill-writing 知識

讀取 `references/writing-guide.md`。這是 skill-writing 知識的單一來源，涵蓋 frontmatter、description 撰寫、progressive disclosure、寫作風格、常見陷阱。
...
跟使用者確認細節。目標是填補空白、解決歧義 — 不是盤問。根據流程複雜度調整提問深度。

### 提問方式

- 每個問題附上建議答案，使用者可以直接同意或調整
- 如果問題可以透過探索 codebase 回答，就自己去看，不要問使用者
- 低爭議的項目合併成一個確認：例如「我建議產出路徑和觸發條件的預設是 [X, Y]，OK？」

### 簡單流程（線性、3-5 步）

問 1-2 個重點問題即可。Extract 階段應該已經收集到大部分資訊。主要確認：

- **觸發條件** — 什麼時候應該用這個 skill？使用者會怎麼呼叫它？
- **泛化** — 這次 session 有什麼東西應該參數化而非 hardcode？

其餘（產出路徑、產出物、步驟順序）提出合理預設值直接走，除非使用者反對。

### 複雜流程（多階段、分支邏輯、重要產出物）

多問一些，但仍然一次一個。除了觸發條件和泛化，額外涵蓋：

- **參數** — skill 需要什麼輸入？
- **步驟調整** — 提煉出的步驟要不要調整順序、合併、或拆分？
- **結束條件** — 每個步驟什麼時候算完成？整個 skill 什麼時候算結束？

### 結束採訪

Claude 跟使用者都可以決定結束。Claude 覺得有足夠資訊可以寫出一個好的初稿時建議結束 — Review 階段會抓到剩下的問題。使用者也可以隨時喊停。

## 階段 5: 撰寫（Write）

結合 `references/writing-guide.md` 的指引，產出完整的 skill 檔案：
...
如果有問題，自行修正或回到階段 4 跟使用者確認。

### 2. 使用者 Review

把產出的 skill 內容呈現給使用者確認。

- 如果使用者有問題或改善建議，視情況回到階段 4 或階段 5
- 如果使用者同意，進入階段 7
